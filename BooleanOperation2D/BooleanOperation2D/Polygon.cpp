...
bool CEdge::IsIntersectWith2(const CEdge& edge) const
{
	if (!Box.IsIntersectWith(edge.Box))
		return false;

	vec2 v1 = End-Start;
	vec2 v2 = edge.End-edge.Start;

	double res = v1^v2;

	if(Tolerance.IsZero(res))
		return false;
	else
	{
		if(Start==edge.Start || Start==edge.End||End==edge.Start||End==edge.End)
			return false;
		else
			return true;
	}
